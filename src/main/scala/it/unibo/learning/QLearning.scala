...
case class QLearning[S, A](actions: NonEmptySet[A], alpha: TimeVariable[Double], gamma: Double) extends ReinforcementLearning[(S, A, Double, S), Q[S, A]] {
  override def improve(trajectory: (S, A, Double, S), q: Q[S, A], clock: Clock): Q[S, A] = {
    ...
    val (_, bestNextValue) = actions
      .toNonEmptyList.map(action => (action, q(stateTPlus, action)))
